...
const DescripcionActividades = ( {actividad, oferente} ) => {
    ...
    const needsTruncation = actividad.itinerario.length > maxTextLength;
    const truncatedText = actividad.itinerario.slice(0, maxTextLength);
    ...
    const handlePrevious = () => {
        setSelectedImgIndex((prevIndex) => {
            const newIndex = prevIndex === 0 ? actividad.imgs.Imagenes.length - 1 : prevIndex - 1;
            setBackgroundImg(actividad.imgs.Imagenes[newIndex]);
            return newIndex;
        });
    };
    ...
    const handleNext = () => {
        setSelectedImgIndex((prevIndex) => {
            const newIndex = (prevIndex + 1) % actividad.imgs.Imagenes.length;
            setBackgroundImg(actividad.imgs.Imagenes[newIndex]);
            return newIndex;
        });
    };
